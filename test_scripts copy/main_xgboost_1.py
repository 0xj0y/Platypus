...
from catboost import CatBoostClassifier  # Changed from XGBoost
...
# -------------------------
# Portfolio Manager with Partial Exits + Trailing + DYNAMIC ALLOCATION
# -------------------------
...
class SwingPortfolioManager:
    def __init__(self,
        ...
        portfolio_increase_threshold: portfolio growth % to trigger allocation increase (e.g., 0.2 = 20%)
        allocation_increment: allocation increase when threshold met (e.g., 0.15 = 15% increase)
        """
        ...
        # NEW: Dynamic allocation parameters
        self.portfolio_increase_threshold = portfolio_increase_threshold
        self.allocation_increment = allocation_increment
        self.current_allocation_multiplier = 1.0  # Start at 100%
        self.last_allocation_update_value = initial_balance  # Baseline for next increase check
    ...
    def _check_and_update_allocation(self):
        ...
        if growth >= self.portfolio_increase_threshold:
            # Increase allocation multiplier
    ...
    def _buy(self, date, symbol, row, capital_per_slot):
        # Check allocation update before buying
        ...
        # Apply current allocation multiplier to capital per slot
        adjusted_capital = capital_per_slot * self.current_allocation_multiplier
        # But don't exceed available balance
        adjusted_capital = min(adjusted_capital, self.balance * 0.9)  # Leave 10% buffer
        ...
        # Handle existing vs new position
        if symbol in self.positions:
            ...
            existing_pos["atr"] = atr  # Use latest ATR
            existing_pos["stop_loss"] = weighted_avg_price - 1.5 * atr  # Recalculate SL
            existing_pos["targets"] = [weighted_avg_price + 1.0 * atr,
                                       weighted_avg_price + 2.0 * atr,
                                       weighted_avg_price + 3.0 * atr]  # Recalculate targets
            # Keep existing milestones_hit
        else:
            # New position
    ...
    for i, current_date in enumerate(unique_days):
        ...
        if model is None or i % retrain_freq_days == 0:
            ...
            if len(train_data) >= 200:
                ...
                # CHANGED: CatBoost instead of XGBoost with equivalent parameters
                ...
                # CHANGED: CatBoost SHAP explainer
...
# -------------------------
# Plot & Report (updated for CatBoost)
# -------------------------
...
def plot_and_report(trades_df, hist_df, training_df, model, explainer, features, master_df, out_dir=PLOTS_DIR):
    ...
    if not trades_df.empty:
        ...
        # NEW: Plot allocation multiplier over time
        ...
        if not buys.empty and "allocation_multiplier" in buys.columns:
            ...
            plt.figure(); plt.plot(buys["date"], buys["allocation_multiplier"], marker='o', markersize=3); plt.title("Allocation Multiplier Over Time"); plt.ylabel("Allocation Multiplier"); plt.savefig(os.path.join(out_dir, "allocation_multiplier.png")); plt.close()
            plt.figure(); sns.histplot(buys["allocation_multiplier"], bins=20); plt.title("Allocation Multiplier Distribution"); plt.savefig(os.path.join(out_dir, "allocation_multiplier_dist.png")); plt.close()
    ...
    # CHANGED: CatBoost feature importance
    try:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, "catboost_fi.png")); plt.close()
    except Exception as e:
        print("Feature Importance failed:", e)

    # SHAP (works with CatBoost)
    try:
