...
def create_sl_loss_report(trades_df, master_df=None):
    ...
    Includes: date, symbol, entry_price, shares, RSI_14, VOL_20, NORM_ATR, volume_ratio, volume_percentile
    """
    ...
    if loss_sl_trades.empty:
        ...
        return pd.DataFrame(columns=["date", "symbol", "entry_price", "shares", "RSI_14", "VOL_20", "NORM_ATR", "volume_ratio", "volume_percentile"])

    # Add extended metrics if master_df provided
    if master_df is not None:
        def find_closest_metrics(row):
            sym = row["symbol"]
            d = row["date"]
            ...
            closest_row = df_sym.loc[closest_idx]
        ...
        loss_sl_trades[["RSI_14", "VOL_20", "NORM_ATR", "volume_ratio", "volume_percentile"]] = loss_sl_trades.apply(find_closest_metrics, axis=1)
    else:
        loss_sl_trades["RSI_14"] = np.nan
        loss_sl_trades["VOL_20"] = np.nan
        loss_sl_trades["NORM_ATR"] = np.nan
        loss_sl_trades["volume_ratio"] = np.nan
        loss_sl_trades["volume_percentile"] = np.nan

    # Select relevant columns for report
    report = loss_sl_trades[["date", "symbol", "entry_price", "shares", "RSI_14", "VOL_20", "NORM_ATR", "volume_ratio", "volume_percentile"]].copy()
    return report
...
def create_win_report(trades_df, master_df=None):
    ...
    if master_df is not None:
        ...
        win_trades[["RSI_14", "VOL_20", "NORM_ATR", "volume_ratio", "volume_percentile"]] = win_trades.apply(find_closest_metrics, axis=1)
    else:
    ...
    # Select relevant columns for report
    report = win_trades[["date", "symbol", "entry_price", "shares", "RSI_14", "VOL_20", "NORM_ATR", "volume_ratio", "volume_percentile"]].copy()
    return report
...
def plot_and_report(trades_df, hist_df, training_df, model, explainer, features, master_df, trailing_sl_after_t3, out_dir=PLOTS_DIR):
    ...
    # ENHANCED: Stop-Loss Loss Report with Extended Metrics
    ...
    if not sl_loss_report.empty:
        ...
        print(f"✅ Enhanced Stop-Loss Loss Report saved: {len(sl_loss_report)} losing SL trades found")
        print(f"   Average RSI at entry for SL losses: {sl_loss_report['RSI_14'].mean():.1f}")
        print(f"   Average VOL_20 for SL losses: {sl_loss_report['VOL_20'].mean():.4f}")
        print(f"   Average NORM_ATR for SL losses: {sl_loss_report['NORM_ATR'].mean():.4f}")
        print(f"   Average Volume Ratio for SL losses: {sl_loss_report['volume_ratio'].mean():.2f}")
        print(f"   Average Volume Percentile for SL losses: {sl_loss_report['volume_percentile'].mean():.2f}")
        
        # Individual histograms for losing trades
        metrics_to_plot = ["RSI_14", "VOL_20", "NORM_ATR", "volume_ratio", "volume_percentile"]
    ...
    # ENHANCED: Winning Trades Report with Extended Metrics
    ...
    if not win_report.empty:
        ...
        print(f"✅ Enhanced Winning Trades Report saved: {len(win_report)} winning trades found")
        print(f"   Average RSI at entry for winning trades: {win_report['RSI_14'].mean():.1f}")
        print(f"   Average VOL_20 for winning trades: {win_report['VOL_20'].mean():.4f}")
        print(f"   Average NORM_ATR for winning trades: {win_report['NORM_ATR'].mean():.4f}")
        print(f"   Average Volume Ratio for winning trades: {win_report['volume_ratio'].mean():.2f}")
        print(f"   Average Volume Percentile for winning trades: {win_report['volume_percentile'].mean():.2f}")
        
        # Individual histograms for winning trades
        metrics_to_plot = ["RSI_14", "VOL_20", "NORM_ATR", "volume_ratio", "volume_percentile"]
        ...
        # 🆚 COMPARATIVE ANALYSIS - Side by Side for All Metrics
        if not sl_loss_report.empty:
...
if __name__ == "__main__":
    ...
    # 🚨 FIXED: Correct unpacking to handle all 8 returned values
